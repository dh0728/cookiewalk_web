...
### 그린 경로의 불확실성 ###
현재 경로분석 알고리즘 연속된 3점을 이용해서 두 개의 백터를 구하고 두 백터의 내적과 외적을 이용해서 구한 각도와 방향을 이용해서 좌회전, 우회전, 유턴을 판단합니다. 하지만 이 경우에 길이 맞는지 아닌지의 여부를 판단할  수 없어 걷는 도중 길이 아닌 경우 이어 걸을 수 없는 경우가 생깁니다.
